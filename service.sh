...
if [ -f "$LOCK_FILE" ]; then
    ...
    if [ "$uptime_secs" -gt 300 ]; then
        log_warn "Lock file found but uptime is ${uptime_secs}s — treating as stale. Clearing and continuing."
        rm -f "$LOCK_FILE"
    else
        log_error "CRITICAL: Lock file found and uptime is only ${uptime_secs}s — genuine bootloop risk."
